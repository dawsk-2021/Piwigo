...
{if get_device() != 'desktop'}
    {combine_script id='jquery.mobile-events' path='themes/daw_theme/node_modules/jQuery-Touch-Events/src/jquery.mobile-events.min.js' require='jquery' load='footer'}
{/if}
...
<div id="main-picture-container" class="container{if $theme_config->fluid_width}-fluid{/if}">
        ...
        {if isset($TITLE_IMG)}
            <div class="text-center col-lg-10 col-md-12 mx-auto">
                <section id="important-info">
                    <h5 class="imageTitle">{$TITLE_IMG}</h5>
                </section>
            </div>
        {/if}
    </div>
    ...
    <div id="theImageComment" class="row justify-content-center mb-3">
        {if isset($COMMENT_IMG)}
            {if strlen($COMMENT_IMG) > 300}
                <div class="text-center col-lg-10 col-md-12 mx-auto">
                    <section id="important-info">
                        <h6 class="imageDescLeft">{$COMMENT_IMG}</h6>
                    </section>
                </div>
            {else}
                <div class="text-center col-lg-10 col-md-12 mx-auto">
                    <section id="important-info">
                        <h6 class="imageDescCenter">{$COMMENT_IMG}</h6>
                    </section>
                </div>
            {/if}
        {/if}
            ...
            <section id="share">
                {if $theme_config->social_twitter}
                    <a href="http://twitter.com/share?text={$current.TITLE}&amp;url={$http_scheme}://{$smarty.server.HTTP_HOST}{$smarty.server.REQUEST_URI}"
                        ...
                        title="Share on Twitter" {if $theme_config->social_buttons}
                        class="btn btn-sm btn-social btn-raised btn-twitter" {/if}>
                ...
                {if $theme_config->social_facebook}
                    <a href="https://www.facebook.com/sharer/sharer.php?u={$http_scheme}://{$smarty.server.HTTP_HOST}{$smarty.server.REQUEST_URI}"
                        ...
                        title="Share on Facebook" {if $theme_config->social_buttons}
                        class="btn btn-sm btn-social btn-raised btn-facebook" {/if}>
                ...
                {if $theme_config->social_pinterest}
                    <a href="https://www.pinterest.com/pin/create/button/?url={$http_scheme}://{$smarty.server.HTTP_HOST}{$smarty.server.REQUEST_URI}{if isset($current.derivatives.large)}&media={$http_scheme}://{$smarty.server.HTTP_HOST}{$smarty.server.REQUEST_URI}/../{$current.derivatives.large->get_url()}{/if}"
                        ...
                        title="Pin on Pinterest" {if $theme_config->social_buttons}
                        class="btn btn-sm btn-social btn-raised btn-pinterest" {/if}>
                ...
                {if $theme_config->social_vk}
                    <a href="https://vkontakte.ru/share.php?url={$http_scheme}://{$smarty.server.HTTP_HOST}{$smarty.server.REQUEST_URI}{if isset($current.derivatives.large)}&image={$http_scheme}://{$smarty.server.HTTP_HOST}{$smarty.server.REQUEST_URI}/../{$current.derivatives.large->get_url()}{/if}"
                        ...
                        {if $theme_config->social_buttons} class="btn btn-sm btn-social btn-raised btn-vk" {/if}>
                    ...
                    {foreach from=$thumbnails item=thumbnail}
                        ...
                        {if $theme_config->photoswipe && !$theme_config->slick_infinite}
                            <div
                                ...
                                <a{if $thumbnail.id eq $current.id} id="thumbnail-active" {/if} href="{$thumbnail.URL}"
                                    data-index="{$idx}" data-name="{$thumbnail.NAME}" data-description="{$thumbnail.DESCRIPTION}"
                                    {if !$theme_config->slick_infinite}data-src-xlarge="{$derivative_xxlarge->get_url()}"
                                        data-size-xlarge="{$derivative_xxlarge->get_size_hr()}"
                                        ...
                                        data-size-original="{$thumbnail.SIZE}" data-video="true" {else}
                                            {if $theme_config->photoswipe_metadata} data-exif-make="{$thumbnail.EXIF.make}"
                                                data-exif-model="{$thumbnail.EXIF.model}" data-exif-lens="{$thumbnail.EXIF.lens}"
                                                data-exif-iso="{$thumbnail.EXIF.iso}" data-exif-apperture="{$thumbnail.EXIF.apperture}"
                                                data-exif-shutter-speed="{$thumbnail.EXIF.shutter_speed}"
                                                data-exif-focal-length="{$thumbnail.EXIF.focal_length}"
                                            data-date-created="{$thumbnail.DATE_CREATED}" {/if} 
                                        {/if} 
                                    {/if}>
                                {else}
                                    <div class="text-center{if $thumbnail.id eq $current.id} thumbnail-active{/if}"><a
                                            href="{$thumbnail.URL}">
                                        {/if}
                                        <img {if $derivative->is_cached()}data-lazy="{$derivative->get_url()}"
                                            {else}data-lazy="{$ROOT_URL}{$themeconf.icon_dir}/img_small.png"
                                            data-src="{$derivative->get_url()}" {/if} alt="{$thumbnail.TN_ALT}"
                                            title="{$thumbnail.TN_TITLE}" class="img-fluid"></a>
                                </div>
                            ...
                            data-size-medium="{$derivative_medium->get_size_hr()}" {if preg_match("/(mp4|m4v)$/", $thumbnail.PATH)}
                                ...
                                {else} 
                                    {if $theme_config->photoswipe_metadata} data-exif-make="{$thumbnail.EXIF.make}"
                                        ...
                                        data-exif-iso="{$thumbnail.EXIF.iso}" data-exif-apperture="{$thumbnail.EXIF.apperture}"
                                        data-exif-shutter-speed="{$thumbnail.EXIF.shutter_speed}"
                                        data-exif-focal-length="{$thumbnail.EXIF.focal_length}" data-date-created="{$thumbnail.DATE_CREATED}"
                                    {/if} 
                                {/if}></a>
                                {assign var=idx value=$idx+1}
                            {/foreach}
